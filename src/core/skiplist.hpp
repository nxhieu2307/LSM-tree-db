...
public:
    ...
    struct Node {
        ...
        Node(std::string k, MemTableEntry e, int level)
            : key(std::move(k)), entry(std::move(e)), forward(level, nullptr) {}
    };
    ...
    void Insert(const std::string& key, const MemTableEntry& entry);

    // Find a key's entry. Returns true if found, and populates entry if not nullptr.
